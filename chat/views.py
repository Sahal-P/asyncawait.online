...
from .serializers import ChatSerializer, ContactsSerializer, MessageUnknownSerializer, GetChatDetailsSerializer, MessageDetailsSerislizer
...
class MessageUnknownAPIView(views.APIView):
    ...
    def post(self, request):
        ...
        chat = Chat.objects.filter(participants=user, is_group_chat=False).filter(participants=second_user).exists()
        if chat:
        ...
        chat.full_clean()
        chat.save()
        contact = Contacts.objects.create(user=user, contact=second_user, is_accepted=True)
...
class GetChatIDAPIView(views.APIView):
    ...
    def post(self, request):
        ...
        chat = Chat.objects.filter(participants__in=[user,second_user], is_group_chat=False).first()
...
class GetChatDetailsAPIView(views.APIView):
    ...
    def post(self, request):
        ...
        chat = Chat.objects.filter(participants=user, is_group_chat=False).filter(participants=chat_user).first()
